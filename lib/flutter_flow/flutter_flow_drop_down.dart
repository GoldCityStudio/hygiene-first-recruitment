...
class _FlutterFlowDropDownState<T> extends State<FlutterFlowDropDown<T>> {
  ...
  List<DropdownMenuItem<T>> _createMenuItems() {
    return widget.options.map((option) {
      final label = optionLabels[option] ?? option.toString();
  ...
  List<DropdownMenuItem<T>> _createMultiselectMenuItems() {
    return widget.options.map((option) {
      final label = optionLabels[option] ?? option.toString();
      return DropdownMenuItem<T>(
            value: option,
            ...
            Checkbox(
              ...
              onChanged: (value) {
                if (value == true) {
                  multiSelectController.value = [...currentValues, option].toList();
                } else {
                  multiSelectController.value =
                      currentValues.where((v) => v != option).toList();
                }
              },
            ),
