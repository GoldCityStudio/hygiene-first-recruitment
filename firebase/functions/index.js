...
exports.onUserDeleted = functions
  ...
  .onDelete(async (user) => {
    ...
    await firestore
      .collection("jobs")
      .where("company_ref", "==", userRef)
      .get()
      .then(async (querySnapshot) => {
        for (var doc of querySnapshot.docs) {
          console.log(`Deleting document ${doc.id} from collection jobs`);
          await doc.ref.delete();
        }
      });
    await firestore
      .collection("applications")
      .where("applicant_ref", "==", userRef)
      .get()
      .then(async (querySnapshot) => {
        for (var doc of querySnapshot.docs) {
          console.log(
            `Deleting document ${doc.id} from collection applications`,
          );
          await doc.ref.delete();
        }
      });
